...
import { getConsentimentoPorToken, type Consentimento, type RespostasAnamnese } from '@/lib/consentimentos'

export default function ConsentimentoFormPage() {
  const params = useParams()
  const token = String(params.token || '')

  const [carregando, setCarregando] = useState(true)
  const [doc, setDoc] = useState<Consentimento | null>(null)
  ...
  useEffect(() => {
    ...
    getConsentimentoPorToken(token)
      .then((d) => {
        setDoc(d)
        if (d?.estado === 'submetido') setSubmetido(true)
        if (d?.clienteNome) setAssinaturaNome(d.clienteNome)
      })
